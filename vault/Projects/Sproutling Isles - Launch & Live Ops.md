...
- [ ] **roblox-engineer:** build the `Config.EventTimeOffset` Studio test hook — blocks all Studio testing of Hollow Harvest until built (event starts 17 Oct, ~3 weeks out). Exact spec, including the 11 call sites to change, is in [[Sproutling Isles - QA Review]]'s "Hollow Harvest QA pass (2026-09-26)" section, part 1: add `Config.EventTimeOffset = 0` (Studio-only, same pattern as `Config.StudioDisableShield`), add `Events.now()` to `src/shared/Events.luau` (`os.time()` plus the offset only inside `RunService:IsStudio()`), then swap every event-phase `os.time()` call in `EventService.luau` (9 sites) and the two call sites in `PlotService.luau`/`PondService.luau` to use `Events.now()` instead. qa-tester's test plan (same section) is ready to run as soon as this lands.
