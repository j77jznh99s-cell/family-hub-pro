...
#### Week 5 code list (live-ops-manager proposal, 2026-09-26)
Replaces the single placeholder `SPROUT1K` demo code in `Codes.luau` (built 2026-09-26, `63c3100`). **Free-tier only, house rule enforced**: every reward below is Dew, a Common/Rare seed (already Dew-purchasable in the base roster, so no new power), or a Luck Boost ≤ 15 min — nothing here is a skin, gamepass or dev-product-gated perk, matching the existing code comment in `Codes.luau` ("Never add a reward here that only paying players could otherwise get"). Amounts are sized well below the store: the smallest dev product is the 29 R$ Dew Pouch (15 min income, min 500 Dew) and the Starter Pack is 49 R$ for 5,000 Dew + 2 seeds — no code below grants Dew or seeds anywhere near that value, since these go to every player for free, unlike a one-time paid purchase. All codes are **evergreen** (no expiry) unless noted, one-time per player (existing `CodesService` behaviour), and go live the moment Week 5 ships (Sat 7 Nov).

| Code | Reward | One-line rationale | Retention window / KPI it serves |
| --- | --- | --- | --- |
| `SPROUT1K` | 1,000 Dew | The announcement copy already promises this exact code — keep it as the simple, evergreen "thank you for finding the Codes button" grant. Value sits between the Dew Pouch's 500 Dew floor and the Starter Pack's 5,000 Dew, so it stays clearly below any real-money offer. | Week 5 KPI: gets players to open the new Codes panel at least once, supporting **days 8–28 play days ≥ 3**. |
| `MISSEDBLOOM` | 1 free Common seed (Mossbun) + 500 Dew | A catch-up code for anyone who joined during Weeks 2–4 and missed the Week 1 Founding Gardener window — softens FOMO with a Common seed and modest Dew, not a rare species, so it's not pay-to-win. Redeemable from Week 5 onward only. | **Day 1 / days 2–7** for latecomers — a low-friction "you didn't miss everything" hook that supports onboarding "first seed bought"/"first hatch" funnel steps for that cohort. |
| `LUCKYGARDEN` | 5-minute Luck Boost | Matches the *shortest* ad-tier Luck reward already planned (section 4, placement 1: 5 min via ad) so a code never beats the free ad path, and stays well under the 49 R$ dev product (15 min). A simple "log in and try a hatch" hook. | **Days 2–7**: hatches per session (same KPI as the Luck kiosk ad placement). |
| `GROWINGSTRONG` | 1 free Rare seed (**Mushroomph**, revised 2026-09-26 — see resolved flag below) + 250 Dew | A milestone/thank-you code for a community moment (e.g. the group or Discord hitting a round member count). **The exact trigger number is a marketing call, not a design one — owner decides when to publish this code**; the reward itself needs no approval since it's free-tier. | **Days 8–28**: gives already-active players (who redeemed `SPROUT1K` in Week 5) a reason to check the Codes panel again later, supporting play days ≥ 3. |
| `SEEDSTASH` | 2 free Common seeds (Mossbun) | The simplest, Dew-free code — pure seed variety for new or alt accounts who redeem late and are still Dew-poor. No inflation risk since Common seeds are cheap and already freely available in the shop. | **Day 1**: reduces early friction on the "first seed bought"/"first plant" onboarding steps for anyone who redeems before their first Dew purchase. |

> [!note] Waiting on the owner
> None of the above needs owner sign-off — codes are free-tier only and don't touch pricing or real money. The only open call is the **publish timing of `GROWINGSTRONG`** (which community milestone/date to tie it to); logged under [[#Waiting on the owner]] below as a marketing decision, not a monetization one.

> [!warning]- Reviewer flag (2026-09-26), **resolved same day** — `GROWINGSTRONG`'s original Rare seed choice
> Checked the actual species data in `Species.luau` (branch `claude/roblox-popular-game-trends-6u7sie`): `mushroomph` (the Rare seed the *paid* 49 R$ Starter Pack grants) has a 120s payback (price 6,000 ÷ dewPerSecond 50); `lilypadger` (the Rare seed this code originally proposed) has a 180s payback (price 18,000 ÷ dewPerSecond 100) — i.e. `lilypadger` is the stronger and pricier of the two Rare seeds, and giving it away free and evergreen would have been a bigger giveaway than the paid Starter Pack's own Rare seed. The Frostbloom Advent Day 4 gift also gives a free `lilypadger`, but only once, on one calendar day, during a bounded 13-day event — not evergreen, so that precedent didn't cover an evergreen code.
>
> **Resolution (2026-09-26, game-designer pass, done directly — a simple pick between two already-scoped options, no agent needed):** swapped the reward to `mushroomph`. Kept the code evergreen (matching every other code on this list) rather than adding one-off expiry logic to `CodesService.luau`, which doesn't support per-code expiry today and shouldn't gain it just for this — `mushroomph` alone, at 120s payback, is no longer priced above the Starter Pack's own Rare grant, so the code is back to "smaller than the cheapest paid pack" like the rest of the list. The table above is now the final, unflagged version.
...
- [ ] Week 5 code `GROWINGSTRONG`: pick which community milestone/date it publishes on (marketing timing only — the reward itself is free-tier and needs no approval). See [[#Week 5 code list (live-ops-manager proposal, 2026-09-26)]].

## Tasks created by this note
- [ ] **roblox-engineer:** analytics wrapper + all events in section 3 (a prerequisite for the beta)
- [ ] **roblox-engineer:** `Config.Boosts` time-boxed boosts (launch weekend, Bloom Rush). *(The `Config.EventTimeOffset` half of this task is now specced precisely below — see the new task after the Summon Spooky Fog dev product line — since qa-tester found it was never actually built during the Hollow Harvest work.)*
...
- [x] **roblox-engineer:** build the `Config.EventTimeOffset` Studio test hook — done 2026-09-26 (`04a3211`, branch `claude/roblox-popular-game-trends-6u7sie`). Added `Config.EventTimeOffset = 0`, an `Events.now()` helper gated on `RunService:IsStudio()`, and swapped all 11 event-phase `os.time()` call sites (9 in `EventService.luau`, 1 each in `PlotService.luau`/`PondService.luau`) to use it. I reviewed the diff myself: all 11 sites confirmed correct, no unrelated `os.time()` calls touched, no require cycle. selene/rojo/lune clean, 1241 parts unchanged, CI green. qa-tester's test plan in [[Sproutling Isles - QA Review]] can now actually be run.
- [ ] **owner:** Studio playtest of the Hollow Harvest event before 17 Oct (not possible from the sandbox) — fast-forward time using the now-built `Config.EventTimeOffset` to confirm the Fog schedule, lantern collection/despawn, stall purchases and caps, the Haunted Greenhouse skin equip, and the end-of-event Moonberry conversion (now fixed to scale with balance). Full step-by-step plan in [[Sproutling Isles - QA Review]].
- [x] **roblox-engineer:** Codes system (free-tier rewards only) for Week 5 — done 2026-09-26 (`63c3100`, branch `claude/roblox-popular-game-trends-6u7sie`). New `Codes.luau` (reward table) + `CodesService.luau` (one-time-per-player validation, reward granting via the existing `State.addDew`/`addSeeds` helpers and the QA-confirmed Luck Boost pattern, toasts, anti-spam cooldown), plus a client Codes button/panel. I reviewed the diff myself — no bugs found. Only the placeholder demo code `SPROUT1K` (1000 Dew, from the announcement copy) exists so far; the real Week 5 code list is a new task below for live-ops-manager, due before 7 Nov. selene/rojo/lune clean, CI green. **Not Studio-tested** (needs the owner, can be folded into the existing play-test task).
- [x] **live-ops-manager:** propose the real Week 5 code list (free-tier rewards only — Dew, common/rare seeds, a short Luck Boost) to replace the `SPROUT1K` placeholder in `Codes.luau` before 7 Nov — done 2026-09-26. 5 codes proposed (`SPROUT1K` kept as-is per the announcement copy, plus `MISSEDBLOOM`, `LUCKYGARDEN`, `GROWINGSTRONG`, `SEEDSTASH`), each tied to a retention window and KPI, all free-tier only (Dew, Common/Rare seeds, ≤5-min Luck Boost) and sized well under the smallest paid dev product. Full table in [[#Week 5 code list (live-ops-manager proposal, 2026-09-26)]]. Vault-only (no `.luau` access) — new task below for roblox-engineer to translate the table into `Codes.luau`'s `Codes.list`.
- [x] **roblox-engineer:** implement the Week 5 code list proposed above into `Codes.luau`'s `Codes.list` — done 2026-09-26 (`da077ea`, branch `claude/roblox-popular-game-trends-6u7sie`). All 5 codes implemented exactly as tabled: `SPROUT1K` (1,000 Dew), `MISSEDBLOOM` (500 Dew + 1 mossbun), `LUCKYGARDEN` (5-min Luck Boost), `GROWINGSTRONG` (250 Dew + 1 mushroomph, the swapped species), `SEEDSTASH` (2 mossbun). I reviewed the diff myself — matches the vault table exactly, no `lilypadger` leaked back in. `CodesService.luau` needed no changes (already handles reward shapes generically). selene/rojo/lune clean, 1241 parts unchanged. **Not Studio-tested** (needs the owner — can be folded into the existing play-test task; test plan: redeem each of the 5 codes, confirm correct grants, confirm one-time-per-player, confirm bad codes toast clearly).
- [x] **game-designer:** confirm/adjust the `GROWINGSTRONG` code's reward — done 2026-09-26 (directly, no agent needed — a simple pick between two already-scoped options). Swapped the reward from `lilypadger` (Rare, 180s payback, priced above the Starter Pack's own Rare grant) to `mushroomph` (Rare, 120s payback, matches the Starter Pack's own seed and is no longer priced above it). Kept the code evergreen rather than adding expiry logic `CodesService.luau` doesn't have. See the resolved reviewer flag in [[#Week 5 code list (live-ops-manager proposal, 2026-09-26)]].
- [ ] **roblox-engineer:** rewarded ads `Ads.luau`, after the owner approves and eligibility is confirmed
- [x] **roblox-engineer:** Founding Gardener badge + Week 8 teaser login tracker — done 2026-09-26 (`35ab411`, branch `claude/roblox-popular-game-trends-6u7sie`). Founding Gardener: `data.foundingGardener` flag set on a brand-new save's first join if it falls in the Week 1 window (10-17 Oct); badge award attempt via a new pcall-wrapped `Badges.luau` facade, no-ops until `Config.FoundingGardenerBadgeId` is a real ID (owner still needs to create it on the Creator Dashboard — same pattern as the 13 Robux items). No title/nameplate UI built (no existing convention to extend). Frostbloom Week 8 teaser: `data.frostbloomTeaser` tracks distinct UTC login-days in the 28 Nov-11 Dec window, `qualified = true` once 3 are recorded — deliberately a data-only tracker; the actual Frostfawn seed grant is a new task below for whoever builds the Frostbloom Advent Gift system. I reviewed the diff and found one real error the agent's own report missed: its Frostbloom-window comment said "Thu 11 Dec 2026" but that date is a **Friday** (independently verified with `date -u`) — the timestamp itself was correct, just the label. Fixed directly (`bb7f395`). selene clean, rojo/lune unchanged (1241 parts). **Not Studio-tested** (both windows are months out; needs a temporary date override to test, same as EventTimeOffset).
- [ ] **(new, unscheduled — Frostbloom build, closer to Dec):** whoever builds the Frostbloom Advent Gift system must check `data.frostbloomTeaser.qualified` on Day 1 (Sat 12 Dec) to grant the Week 8 teaser's free Frostfawn seed to qualifying players.
...
- [x] **qa-tester:** event test plan using `EventTimeOffset` (start, Part 2, Witching Hour, end conversion), Starter Pack repeat purchase check, analytics once-only check — done 2026-09-26, see [[Sproutling Isles - QA Review]]'s "Hollow Harvest QA pass" section. **`EventTimeOffset` does not exist yet** — confirmed no such `Config` field and 11 raw `os.time()` call sites gating event phase across `EventService.luau`, `PlotService.luau` and `PondService.luau`; a precise fix spec (an `Events.now()` helper + `Config.EventTimeOffset`, Studio-only) is written up there, with the exact call sites to swap, and a full test plan written assuming the hook exists — none of it runnable until the hook is built (new task below). Starter Pack: confirmed unchanged, m15 still open, not a regression from today's analytics/Hollow Harvest work. Analytics: all 11 onboarding once-only guards confirmed correct, no double-fire risk found in today's Hollow Harvest or leaderboard changes; one documentation nit noted (EventShop's `Opened` funnel step fires per buy-attempt, not per "shop open" — reads as intentional, matches the Store funnel's own documented workaround, but isn't commented at its own call site).
