...
> [!warning] Reviewer flag (2026-09-26): `GROWINGSTRONG`'s Rare seed choice needs a game-designer look before this gets implemented
> Checked the actual species data in `Species.luau` (branch `claude/roblox-popular-game-trends-6u7sie`): `mushroomph` (the Rare seed the *paid* 49 R$ Starter Pack grants) has a 120s payback (price 6,000 ÷ dewPerSecond 50); `lilypadger` (the Rare seed this *free, evergreen* code proposes) has a 180s payback (price 18,000 ÷ dewPerSecond 100) — i.e. `lilypadger` is the stronger and pricier of the two Rare seeds, and this code would hand it out for free, forever, to every player who redeems it, one time each. That's a bigger giveaway than the paid Starter Pack's own Rare seed. The Frostbloom Advent Day 4 gift also gives a free `lilypadger`, but only once, on one calendar day, during a bounded 13-day event — not evergreen. Every *other* code on this list is Dew, a Common seed, or a short Luck Boost, all comfortably below any paid item; `GROWINGSTRONG` is the one exception, and it's an exception in the wrong direction (bigger than the cheapest paid pack, not smaller). Two reasonable fixes, either works: (a) swap the reward to `mushroomph` (still a nice free Rare, but no longer priced above the Starter Pack's own Rare grant), or (b) keep `lilypadger` but time-box the code to a short redemption window instead of evergreen (matching the Advent precedent's one-day scarcity), rather than a permanent evergreen offer. Not fixing this directly since sizing a Rare seed's exact anti-inflation ceiling is a game-designer call, not something to silently override — flagged as a new queue task instead.
...
- [ ] **roblox-engineer:** implement the Week 5 code list proposed above into `Codes.luau`'s `Codes.list` (replacing/extending the `SPROUT1K`-only placeholder), matching the existing one-time-per-player, free-tier-only pattern already built in `CodesService.luau`. **Blocked on the game-designer task below for `GROWINGSTRONG` specifically** — the other 4 codes (`SPROUT1K`, `MISSEDBLOOM`, `LUCKYGARDEN`, `SEEDSTASH`) have no open flags and could be implemented now if this one's held back.
- [ ] **game-designer:** confirm/adjust the `GROWINGSTRONG` code's reward — see the reviewer flag directly above the code table in [[#Week 5 code list (live-ops-manager proposal, 2026-09-26)]]. It proposes a free evergreen `lilypadger` (Rare, 180s payback), which is priced *above* the Starter Pack's own paid Rare seed (`mushroomph`, 120s payback) — the one code on this list that's bigger than a paid item instead of smaller. Pick one: swap to `mushroomph`, or keep `lilypadger` but make the code time-boxed instead of evergreen.
